...
console.log(team);
			 ...
			 Player.findOne({"Position": pos, "Team": team}, function (err, players) {
				if(err) {
					...
					res.render('index', {AaronRodgers: final_player});
				} else {res.render('index', {AaronRodgers: "No results found"});}
				
			
			}) //closes player.findone
	}

	function pos_parse(input, res) {
			 pos = "";
			 team = "";
...
app.post('/', function(req, res){
	...
	if(username == ""){
		res.render('index', {AaronRodgers: "Please Enter a Search"});
	}
	else {var result = pos_parse(username, res);}
