...
$(".jumbotron button").on("click", function(e) {
	var input = document.getElementById("search-bar2345").value;
	String(input);
...
//Team var setting	
	...
	dbQuery(pos, team);
			...
			//query DB and store in var, then pass to functions
			//addToDoc(dbResult);
	
/*function addToDoc(dbResults) {
	$(".result-text").append("<img class='result-pic' src='assets/images/rodgers.jpg' alt='Aaron Rodgers Roster Picture' height='200' width='200'/> <p class='result-text'>Aaron Rodgers</p>");
};*/
...
/*db.nfl.find({Position: pos, Team: team}, function(err, nfl) {
	...
	else nfl.forEach( function(player){alert(player);});
});*/
	console.log(team);
	console.log(pos);
}
